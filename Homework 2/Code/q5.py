...
mu    = 200 # mean of gaussian, w_0
sigma = 20 # standard deviation of gaussian, sigma_w
...
x1       = generate_poisson(N, r, dt, M) # generate the spiking pattern S1
current1 = generate_current(N, x1, epsp) # compute postsynaptic currents for S1
x2       = generate_poisson(N, r, dt, M) # generate the spiking pattern S2
current2 = generate_current(N, x2, epsp) # compute postsynaptic currents for S2
...
def increase_synapticWeights(x, w, N, maxIndex):
  for i in range(N):
...
def decrease_synapticWeights(x, w, N, maxIndex):
  for i in range(N):
    spikes = np.where(x[i] > 0)[0] # indices where neuron spiked
    if(spikes.size == 0):
      continue
...
def increaseS1():
  global w
  I_post = np.transpose(np.multiply(np.transpose(current1), w)) # compute weighted postsynaptic currents
  I = np.sum(I_post, axis = 0) # compute total postsynaptic current
  V = compute_postV(I, M, dt)
  max_iter = 100 # maximum number of iterations
  j = 1 # flag: 1 if no weight updates are made, 0 otherwise
  ...
  for i in range(max_iter):
    ...
    w = increase_synapticWeights(x1, w, N, maxIndex)
    # Recompute postsynaptic current and voltage  
    I_post = np.transpose(np.multiply(np.transpose(current1), w)) # compute weighted postsynaptic currents
    I = np.sum(I_post, axis = 0) # compute total postsynaptic current
    V = compute_postV(I, M, dt)
    j = 0 # atleast one weight update has happened
  return -1


def decreaseS2():
  global w
  I_post = np.transpose(np.multiply(np.transpose(current2), w)) # compute weighted postsynaptic currents
  I = np.sum(I_post, axis = 0) # compute total postsynaptic current
  V = compute_postV(I, M, dt) 
  max_iter = 100 # maximum number of iterations
  j = 1 # flag: 1 if no weight updates are made, 0 otherwise
  ...
  for i in range(max_iter):
    ...
    for j in maxIndex: # iterate over all spikes
      w = decrease_synapticWeights(x2, w, N, j)
    # Recompute postsynaptic current and voltage  
    I_post = np.transpose(np.multiply(np.transpose(current2), w)) # compute weighted postsynaptic currents
    I = np.sum(I_post, axis = 0) # compute total postsynaptic current
    V = compute_postV(I, M, dt)
...
# Compute responses for S1 before updates
I_post = np.transpose(np.multiply(np.transpose(current1), w)) # compute weighted postsynaptic currents
I = np.sum(I_post, axis = 0) # compute total postsynaptic current
V1_i = compute_postV(I, M, dt)
# Compute responses for S2 before updates
I_post = np.transpose(np.multiply(np.transpose(current2), w)) # compute weighted postsynaptic currents
I = np.sum(I_post, axis = 0) # compute total postsynaptic current
V2_i = compute_postV(I, M, dt) 

max_iter = 100 # maximum number of iterations
# Iterate until the neuron spikes for S1 and not for S2
for i in range(max_iter):
  v1 = increaseS1()
  if(v1 == -1):
    print "Could not make neuron spike for S1!"
    break
  v2 = decreaseS2()
  if(v2 == -1):
    print "Could not suppress spikes for S2!"
    break
  if (v1 == 1 and v2 == 1):
    print "Valid weights found"
    print "Number of iterations required: ", i + 1
    break


# Plot response for S1 before weight updates
plt.plot(1e3 * dt * np.arange(M), V1_i[:, 0])
plt.title('AEF RS Neuron for S1', fontweight='bold')
plt.ylabel('Membrane Potential (V)', fontweight='bold')
plt.xlabel('Time (msec)', fontweight='bold')
plt.show()

# Plot response for S2 before weight updates
plt.plot(1e3 * dt * np.arange(M), V2_i[:, 0])
plt.title('AEF RS Neuron for S2', fontweight='bold')
plt.ylabel('Membrane Potential (V)', fontweight='bold')
plt.xlabel('Time (msec)', fontweight='bold')
plt.show()

# Plot response for S1 after updates
I_post = np.transpose(np.multiply(np.transpose(current1), w)) # compute weighted postsynaptic currents
I = np.sum(I_post, axis = 0) # compute total postsynaptic current
V = compute_postV(I, M, dt)
...
# Plot response for S2 after updates
I_post = np.transpose(np.multiply(np.transpose(current2), w)) # compute weighted postsynaptic currents
I = np.sum(I_post, axis = 0) # compute total postsynaptic current
V = compute_postV(I, M, dt) 
plt.plot(1e3 * dt * np.arange(M), V[:, 0])
