...
T  = 2000 * 1e-3 # total time to simulate (sec)
...
def updateSynapticWeights(i, t):
  ...
  for j, syn in enumerate(w_d[i]):
    ...
    if(i >= N_exc): # check if the neuron is inhibitory
      dw = -1 * dw
      if(dw > 0):
        print "Error"
    w_d[i][j][1] += dw
...
for t in range(M-1):
  ...
  for k in range(N - N_exc):
    wi = w_d[N_exc + k]
    for r in wi:
      wi_avg[t] += r[1] / ( (N - N_exc) * fan)  
...
plt.xlabel('Time (msec)', fontweight='bold')
plt.show()
# Plot average inhibitory weight
plt.plot(1e3 * dt * np.arange(len(wi_avg) - 1), wi_avg[:M-1])
plt.title('Average inhibitory weight', fontweight='bold')
plt.ylabel('Weight', fontweight='bold')
plt.xlabel('Time (msec)', fontweight='bold')
plt.show()